...
@xlpro.register_sub
def generate_invoice(thiswb: 'xl._Workbook'):

    payment_due_days = 30

    sht:'xl.Worksheet' = thiswb.ActiveSheet
    ...
    date_due = (date + datetime.timedelta(payment_due_days))
    invoice_table:'xl.ListObject' = sht.ListObjects("Invoice_Table")

    document_uid = f"{client_code}_{project_number}_INV_{invoice_number}_{revision}"

    df_invoice_table = pd.DataFrame(invoice_table.DataBodyRange.Value, columns=invoice_table.HeaderRowRange.Value[0])
    ...
    def update_header():
        funcs_vals = [
            ...
            (doc.Bookmarks("DETAILS_BILLED_TO").Range, "\n".join([bill_to_name, bill_to_email, bill_to_company, bill_to_addr1, bill_to_addr2, bill_to_addr3]), "DETAILS_BILLED_TO"),
            ...
            (doc.Bookmarks("DUE_DATE").Range, date_due.strftime('%Y-%m-%d'), "DUE_DATE"),
            (doc.Bookmarks("ADDITIONAL_INFORMATION").Range, additional_info, "ADDITIONAL_INFORMATION"),
    ...
    def update_invoice_table():
        ...
        for i, row in enumerate(df_invoice_table.values):
            for j, val in enumerate(row):
                if j in [3, 4]:
                    x = (invoice_table_wd.Rows(2+i).Cells(j+1).Range, f"£\t{val:.2f}", None)
                elif j == 0:
                    x = (invoice_table_wd.Rows(2+i).Cells(j+1).Range, f"{int(val)}", None)
                elif j == 2:
                    x = (invoice_table_wd.Rows(2+i).Cells(j+1).Range, f"{int(val)}", None)
                else:
